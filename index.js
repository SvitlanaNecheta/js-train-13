...
function complexConvert(data) {
  let rezult=Object();// Створюємо новий порожній об'єкт для збереження результату.
  const obj=JSON.parse(data);// Перетворюємо json дані в об'єкт та отримуємо всі ключі об'єкта.
  for(let key in obj){// Обходимо всі ключі та перевіряємо значення.
    if(typeof(obj[key])==='number'){ obj[key]=obj[key]+1}}
  rezult=JSON.stringify(obj);// Якщо значення є числом, збільшуємо його на 1.
  // Якщо значення не є числом, просто копіюємо його у новий об'єкт без змін.
 return rezult; // Повертаємо оброблений об'єкт.
}
...
function manipulateUrl(url) {
  let url1=new URL(url); // Створюємо новий об'єкт URL.
  url1.protocol='https:';// Змінюємо протокол URL на https.
  url1.host='newhost.com';// Змінюємо хост URL на 'newhost.com'.
  url1.searchParams.set('newParam','newValue');// Додаємо параметр 'newParam' зі значенням 'newValue' до URL.
  if(url1.searchParams.has('oldParam')){ url1.delete('oldParam');};// Видаляємо параметр 'oldParam' з URL, якщо він існує.
  return url1;// Повертаємо об'єкт, який містить різні властивості URL.
}
...
const url1=new URL(url);// Створення нового об'єкта URL з вхідного рядка
const param = url1.searchParams; // Отримання об'єкта URLSearchParams з властивості 'searchParams' об'єкта URL
 console.log(param);
const map=new Map([]); // Створення порожнього словника для збереження параметрів пошуку
let i=0;
 map.forEach((el)=>{el=param});// Перебір кожного параметра пошуку з 'searchParams' та додавання їх до словника 'params'
 return map; // Кожен 'param' - це масив, де [0] - ім'я параметра, а [1] - значення параметра
  // Повертаємо словник
}
...
function manipulateSearchParams(paramsObj, newUrl) {
  const url1=new URL(newUrl);// Створюємо новий об'єкт URL з нової URL-адреси.
  let keys=Object.keys(paramsObj);// Використовуючи метод 'keys' з об'єкта Object, отримуємо всі ключі paramsObj.
  for(let key of keys){url1.searchParams.append(key,paramsObj[key])}// За допомогою циклу 'for of' перебираємо всі ключі та додаємо параметри пошуку до urlObj.
  return url1.toString();// Повертаємо нову URL-адресу в рядковому форматі.
}
...
function deleteSearchParams(keys, url) {
  let url1=new URL(url);// Створюємо новий об'єкт URL з URL-адреси.
  for(let key of keys){
    if(url1.searchParams.has(key)){url1.searchParams.delete(key);}
...
function createURLWithParams(params, url) {
  let newUrl=new URL(url);// Створюємо новий об'єкт URL з базової URL-адреси.
  for(const key in params){ newUrl.searchParams.set(key,params[key])}; // За допомогою циклу 'for in' перебираємо всі ключі та значення об'єкта params та додаємо їх як параметри пошуку до urlObj.
 return newUrl.toString(); // Повертаємо нову URL-адресу в рядковому форматі.
}
...
function updateURLHash(url, hash) {
 let urlNew=new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
 urlNew.hash=hash; // Оновлюємо значення хеша в URL-адресі.
  return urlNew.toString();// Повертаємо нову URL-адресу в рядковому форматі.
}
...
function appendSearchParam(url, key, value) {
  let urlNew=new URL(url);// Створюємо новий об'єкт URL з вхідної URL-адреси.
  urlNew.searchParams.append(key,value);// Додаємо новий параметр пошуку до URL-адреси.
  return urlNew.toString();// Повертаємо нову URL-адресу в рядковому форматі.
}
...
function modifyURLParameters(url, params) {
 let newUrl=new URL(url); // Створюємо новий об'єкт URL з вхідної URL-адреси.
 // newUrl.searchParams.set(el);
  for(const param in params){
    if(newUrl.searchParams.has('param')){newUrl.searchParams.set(param,params[param])}
    else{
    newUrl.searchParams.append(param,params[param])}
  };// Перебираємо словник params за допомогою for in
 return newUrl.toString();// Отримаємо значення словника по ключу
...
function checkURLParameters(url, params) {
  let newUrl=new URL(url);// Створюємо новий об'єкт URL з вхідної URL-адреси.
  let rez=Object();// Створюємо новий об'єкт для зберігання результатів.
  for(let param of params){
    if(newUrl.searchParams.has(param)){rez[param]=true}
    else{rez[param]=false}
  }// Перебираємо елементи множини params за допомогою for of.
  return rez;// Додаємо новий ключ в результат з булевим значенням, яке вказує, чи є параметр в URL.
...
function processUrl(url, options) {
  ...
  if(options.searchParams!==undefined){// Перевіряємо, чи в об'єкті 'options' є параметри пошуку.
  for(const option in options.searchParams)// Якщо є, перебираємо ці параметри за допомогою циклу 'for in'.
  {  urlObj.searchParams.append(option,options.searchParams[option])};}// Для кожного ключа параметру додаємо його та відповідне значення до об'єкта 'urlObj' за допомогою методу 'append'.
  if(options.protocol!==undefined){urlObj.protocol=options.protocol};// Перевіряємо, чи в об'єкті 'options' є протокол.
  if(options.host!==undefined){urlObj.host=options.host};// Якщо є, змінюємо протокол 'urlObj' на протокол з 'options'.
...
function manipulateQuery(url, options) {
 ...
 // Якщо в словнику `options` є ключ `append`...
  ...
  for(const [key,value] of options){
    ...
    if(options.has('append')){newUrl.searchParams.append(key,value);};// ...перебираємо його ключі та значення за допомогою циклу for...of.
  
      if(options.has('delete')){ newUrl.searchParams.delete(key,value);
    } 
  };// Додаємо кожний ключ і значення до об'єкта `searchParams` в URL.
  return newUrl.toString();// Якщо в словнику `options` є ключ `delete`...
...
function getUrlData(url) {
  let newUrl=new URL(url);// Створюємо новий об'єкт URL з вхідної URL-адреси.
  let obj= {origin: newUrl.origin,
    hostname: newUrl.hostname,
    port: newUrl.port,
    username: newUrl.username,
    password: newUrl.password
  };
  return obj;// Повертаємо об'єкт з відповідними даними.
}
1
...
function sortUrlParams(url) {
  let newUrl=new URL(url);// Створюємо новий об'єкт URL з вхідної URL-адреси.
  newUrl.searchParams.sort(URLSearchParams);// Отримуємо масив з ключами і значеннями параметрів за допомогою методу 'entries'.
  return newUrl.toString();// Сортуємо масив за ключами у порядку зростання.
...
function getURLValues(url) {
  let newUrl=new URL(url);// Створюємо новий об'єкт URL з вхідною URL-адресою.
 let searchP=newUrl.searchParams; // Отримуємо об'єкт `URLSearchParams` з пошуковими параметрами.
   // Отримуємо масив ключів пошукових параметрів.
  let arr=Array();
  let i=0;
  for(let key of searchP.keys()){arr[i]=searchP.getAll(key); i++};// Масив для збереження значень пошукових параметрів.
  return arr;// Перебираємо ключі пошукових параметрів.
...
function getUrlKeys(url) {
  let newUrl= new URL(url);// Створюємо новий об'єкт URL з вхідної URL-адреси.
  let arr=Array();
  let i=0;// Отримуємо масив зі всіма ключами пошукових параметрів за допомогою методу 'keys'.
 for(let key of newUrl.searchParams.keys()){
   arr[i]=key; i++;
